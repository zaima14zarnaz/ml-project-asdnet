...
def load_inference_data(dataset, image_id, config):
    ...
    # Convert to float32 before resizing (OpenCV does not support bool)
    obj_spatial_masks = obj_spatial_masks.astype(np.float32)

    num_masks = obj_spatial_masks.shape[2]
    resized_masks = np.zeros((32, 32, num_masks), dtype=np.float32)

    for i in range(num_masks):
        resized_masks[:, :, i] = cv2.resize(obj_spatial_masks[:, :, i], (32, 32))

    # [32, 32, N] -> [N, 32, 32, 1]
    obj_spatial_masks = np.expand_dims(np.transpose(resized_masks, [2, 0, 1]), -1)

    batch_obj_spatial_masks = np.zeros(
        shape=(config.SAL_OBJ_NUM, 32, 32, 1), dtype=np.float32
    )
    batch_obj_spatial_masks[:obj_spatial_masks.shape[0]] = obj_spatial_masks
