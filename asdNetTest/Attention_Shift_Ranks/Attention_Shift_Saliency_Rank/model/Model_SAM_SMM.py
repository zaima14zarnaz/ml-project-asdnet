...
#         for i in range(4)
#     ]
#     ln2_stack = [layers.LayerNormalization(name=f"cross_attn_ln2_{i+1}") for i in range(4)]
#     ffn_stack = [
#         tf.keras.Sequential([
...
#         for i in range(4)
#     ]
#     ln3_stack = [layers.LayerNormalization(name=f"cross_attn_ln3_{i+1}") for i in range(4)]
...
#                 hist_stack = tf.stack(history_feats[-2:], axis=1)   # (B, 2, F)
...
#             # Transformer-style stack: cross-attn → LN → FFN → LN
#             attn_in = query_hist
#             for i in range(4):
#                 attn_out = mha_stack[i](query=attn_in, value=final_obj_feat, key=final_obj_feat)
#                 attn_in = ln2_stack[i](attn_in + attn_out)    # residual + LN

#                 ffn_out = ffn_stack[i](attn_in)
#                 attn_in = ln3_stack[i](attn_in + ffn_out)     # residual + LN
