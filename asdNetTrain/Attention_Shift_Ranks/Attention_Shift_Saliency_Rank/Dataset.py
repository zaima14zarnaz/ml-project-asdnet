...
class Dataset(object):
    def __init__(self, dataset_root, pre_proc_data_dir, data_split):
        ...
        print(f'Pre processed data root: {self.pre_proc_data_dir_root}')
    ...
    def load_rank_order_data(self, rank_order_root):
        ...
        rank_order_data_files = sorted(
            [f for f in os.listdir(rank_order_root) if os.path.isfile(os.path.join(rank_order_root, f))]
        )
    ...
    def load_object_seg_data(self, obj_data_path):
        ...
        for i in range(len(obj_bbox)):
            # print(f'bbox len: {len(obj_bbox)}')
            # print(f'ranks available: {len(self.gt_rank_orders)}')
            # print(i)
    ...
    def load_object_roi_masks(self, image_id, sel_non_sal_obj):
        ...
        for i in range(len(rois)):
            ...
            obj_mask[int(o_y1):int(o_y2), int(o_x1):int(o_x2)] = 1
    ...
    #     required = ["obj_feat", "shuffled_indices",
    #                 "sel_not_sal_obj_idx_list", "chosen_obj_idx_order_list", "P5"]
    #     if any(k not in obj_data for k in required):
    #         return None, None, None, None, None, None
    ...
    #     gt_ranks = []
    #     for o_idx in shuffled_indices:
    #         if o_idx == -99:
    #             gt_ranks.append(0)
    #         else:
    #             rank_order = rank_orders[o_idx]
    #             if rank_order > 0.5:
    #                 rank_cls = int(11 - (rank_order * 10))  # 1..10
    #                 gt_ranks.append(rank_cls)
    #             else:
    #                 gt_ranks.append(0)
