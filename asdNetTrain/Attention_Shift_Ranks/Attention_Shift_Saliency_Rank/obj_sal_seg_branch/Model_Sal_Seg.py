...
def build_saliency_seg_model(config, mode):
    ...
    if mode == "training":
        ...
        image_shape = tf.shape(input_image)[1:3]                      # int32 tensor
        image_shape = tf.cast(image_shape, tf.float32)                # if your norm uses float division

        gt_boxes = layers.Lambda(
            lambda t: model_utils.norm_boxes_graph(t[0], t[1])
        )([input_gt_boxes, image_shape])
    ...
    # Anchors
    if mode == "training":
        anchors_np = get_anchors(config, config.IMAGE_SHAPE)   # shape [N, 4]
        anchors_tf = tf.constant(anchors_np, dtype=tf.float32) # constant, not Variable

        def anchors_for_batch(img):
            b = tf.shape(img)[0]                 # dynamic batch size
            a = tf.expand_dims(anchors_tf, 0)    # [1, N, 4]
            return tf.tile(a, [b, 1, 1])         # [B, N, 4]

        anchors = layers.Lambda(anchors_for_batch, name="anchors")(input_image)
    else:
