...
class Obj_Sal_Seg_Dataset(object):
    ...
    def load_obj_gt_mask(self, image_id):
        ...
        # print(f"[dbg] image_id={image_id}, N={len(obj_seg)}, sal_obj_idx={list(sal_obj_idx)}")
        ...
        # for s_i in range(min(len(sal_obj_idx), len(obj_seg))):
        ...
        # obj_masks = np.stack(instance_masks, axis=2).astype(bool)
        # class_ids = np.array(class_ids, dtype=np.int32)

        # return obj_masks, class_ids
        # obj_seg is the list of per-object segmentations for this image
        N = len(obj_seg)

        # get a working copy of the provided indices
        idxs = list(sal_obj_idx)

        # If they look 1-based (min==1 and max <= N), convert to 0-based
        if idxs and min(idxs) == 1 and max(idxs) <= N:
            idxs = [i - 1 for i in idxs]

        # Keep only valid, unique, sorted indices
        valid_idx = sorted({i for i in idxs if 0 <= i < N})

        if len(valid_idx) != len(idxs):
            print(f"[warn] image_id={image_id} N={N} dropped {len(idxs)-len(valid_idx)} invalid "
                f"indices (orig={idxs}, kept={valid_idx})")

        # If nothing valid remains, return empty masks
        if not valid_idx:
            return np.zeros((height, width, 0), dtype=bool), np.zeros((0,), dtype=np.int32)

        instance_masks, class_ids = [], []
        for sal_idx in valid_idx:
            seg = obj_seg[sal_idx]
            mask = utils.get_obj_mask(seg, height, width)
            instance_masks.append(mask)
            class_ids.append(1)
