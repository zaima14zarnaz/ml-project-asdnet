...
def get_usable_salient_objects_agreed(image_1_list, image_2_list):
    ...
    for idx in range(len(image_1_list)):
        if idx >= len(image_2_list):
            print(f"[Error] idx {idx} out of range for image_2_list of length {len(image_2_list)}")
            continue  # or break, or handle safely
        v = image_1_list[idx]
        v2 = image_2_list[idx]
